...
- Ingest two `xr.Dataset` objects as inputs, one as a {term}`reference` and another as a {term}`comparison`.
- Return a `pd.DataFrame` that will contain any {term}`scalar` or {term}`score` generated by the analysis. The dataframe can have as many rows/scalars as you like, but the `ilamb3` system will expect specific columns, which we will explain below.

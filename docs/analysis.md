...
An analysis can be **general**, or it can be **specialized** to use particular variables, regions, product types (gridded and/or point), or dimensions (time, space, etc.). ILAMB employs both. For example, the [bias](https://github.com/rubisco-sfa/ilamb3/blob/main/ilamb3/analysis/bias.py) and [rmse](https://github.com/rubisco-sfa/ilamb3/blob/main/ilamb3/analysis/rmse.py) analysis functions are general and can be applied to a wide range datasets, while the [runoff sensitivity](https://github.com/rubisco-sfa/ilamb3/blob/main/ilamb3/analysis/runoff_sensitivity.py) and [nbp](https://github.com/rubisco-sfa/ilamb3/blob/main/ilamb3/analysis/nbp.py) analysis functions are specialized and require specific variables. Further, functions can be **simple** or **robust**. A **simple** function is prone to breakage, while a **robust** function contains logic that handles various input data and edge cases. The process of writing an analysis function follows a natural progression:
...
- Take two `xr.Dataset` objects as inputs, one as a {term}`reference` (`ref`) and another as a {term}`comparison` (`com`).
- Return a `pd.DataFrame` that will contain any {term}`scalar` or {term}`score` values generated by the analysis. The dataframe can have as many rows/scalars as you like, but the `ilamb3` system will expect specific columns, which we will explain below.
...
- `source`: The data source that the scalar is referencing: either the `Reference` or `Comparison` dataset. If your scalar is representative of both datasets, you should choose `Comparison` as the `source` because inherently the scalar is meant to evaluate the performance of the comparison dataset against the reference dataset.
...
- `type`: The type of scalar being reported, either `scalar` or `score`.
    - A {term}`scalar` is any statistic you compute; they could represent errors, deviations, means, etc. It can be any value that you think is important to report, and it can be in any units. They will be displayed in the scalars table on the dataset pages.
    - A {term}`score` is a synthesis of scalars to make it easier to compare performance across models and datasets. They are dimensionaless and values range from 0 to 1 where 1 is considered perfect. If you provide scores, ILAMB will use them to produce an additional "Overall Score."
...
- We add a call to `clip()` to the `eps_gridded` computation and specify a small minimum value to avoid division by zero errors.
...
- We created a member function named `__init__` and moved our `varname` argument to it.Python has a number of *double under* (a.k.a *dunder*) methods that have special behavior. The `__init__` method is called when an instance of the class is created, and it allows us to initialize the instance's attributes. Also note that we have renamed `varname` to `required_variable` as this is what `ilamb3` will pass into the constructor.
...
You should see the previous informational output lines when you run your command. If an error occurs, you will see the [stack trace](https://en.wikipedia.org/wiki/Stack_trace) explaining the error. If the code ran successfully, you should be able to see the output files in the `my_analysis_test` directory:

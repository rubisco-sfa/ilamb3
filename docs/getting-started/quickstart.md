...
This is a gridded product of `gpp` that was generated by Columbia University using the WECANN model and then formatted for use in ILAMB.
...
1. Using a key from one of the ILAMB data [catalogs](../reference/datasets.md).
...
This will download any keys it finds that are not already downloaded. Using data from our [catalogs](../reference/datasets.md) is a great way for you to build benchmarking comparisons that are portable to other systems and shareable with your colleagues. If there are data that you want to use that are not in our registries and you think may be useful to the broader community, you can [format](https://github.com/rubisco-sfa/ilamb3-data/blob/main/docs/how-to-contribute-data.md) the data to be ILAMB-ready and submit a [pull request](https://github.com/rubisco-sfa/ilamb3-data/pulls) in our `ilamb3-data` GitHub repository. If approved, your data will be added to the public registry and made available for anyone to use in their benchmarking studies.
...
If you want to benchmark CMIP model data, you can use [intake-esgf](https://doi.org/10.5281/zenodo.18378994) to query and download CMIP data. See this [tutorial](../tutorials/intake.md) that queries and downloads CMIP model data using intake-esgf, and then creates a CSV that ILAMB can read.
```
...
3. Contain the columns `source_id`, `member_id`, and `grid_label`, unless you have [configured](../tutorials/global-options.md) `ilamb3` otherwise. If you are using your own model data that has not been standardized, you may have to create these columns manually and fill with data you invent.

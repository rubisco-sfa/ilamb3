...
def api():
    ...
    (docs_dir / "ilamb3-api.json").write_text(json.dumps(api_data, indent=2) + "\n")
...
""")
    ...
    module_links = []
    for module in modules:
        module_links.append(f"- [`ilamb3.{module}`](./api/{module}.md)")
        for function in sorted(
            ...
            module_links.append(f"  - [`{function}`](./api/{module}.md#{target})")
    module_links = "\n".join(module_links)
    index = f"""---
# Generated by setup_doc_assets.py; do not edit.
title: API Reference
...
{module_links}
"""
    (docs_dir / "api.md").write_text(index)

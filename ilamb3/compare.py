...
def nest_spatial_grids(*args):
    ...
    *args
    ...
    out = []
    for arg in args:
        iarg = {dset.get_dim_name(arg, "lat"): lat, dset.get_dim_name(arg, "lon"): lon}
        # the interp function will strip the units off the args, so we do some pint
        # gymnastics to avoid warnings
        iarg = arg.pint.dequantify().interp(iarg, method="nearest").pint.quantify()
        # if 'bounds' existed, they will now be interpolated and incorrect
        for dim in ["lat", "lon"]:
            dim_name = dset.get_dim_name(iarg, dim)
            dim = iarg[dim_name]
            if "bounds" in dim.attrs and dim.attrs["bounds"] in iarg:
